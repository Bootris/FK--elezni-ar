...
echo "== Preduslovi =="
php -m | grep -qi pdo_sqlite && ok "PHP pdo_sqlite ekstenzija" || bad "PHP pdo_sqlite ekstenzija nedostaje"
...
if php artisan migrate:status >/dev/null 2>&1; then
    ...
    bad "ne mogu da otvorim bazu (database/database.sqlite, DB_* u .env)"
fi

ADMIN_PATH=$(grep -E '^ADMIN_PATH=' .env 2>/dev/null | cut -d= -f2- | tr -d '"')
...
check_url /sr 200
check_url /en 200
check_url /sr/prvi-tim 200
check_url /sr/omladinci 200
check_url /sr/podrzi-klub 200
check_url /sr/kontakt 200
check_url /vesti 200
check_url /video 200
...
curl -s "$BASE/sr" | grep -q 'Upiši se' \
    && ok "početna renderuje CTA za upis" \
    || bad "početna ne renderuje očekivani sadržaj"

curl -s "$BASE/$ADMIN_PATH/login" | grep -qi 'lozinka' \
