...
class SitemapController extends Controller
{
    public function index()
    {
        ...
        foreach (['sr', 'en'] as $locale) {
            ...
            foreach (['prvi-tim', 'omladinci', 'podrzi-klub', 'kontakt'] as $page) {
                $xml .= $this->url("{$baseUrl}/{$locale}/{$page}", now()->toISOString(), 'weekly', '0.8');
            }
        }

        $xml .= $this->url("{$baseUrl}/vesti", now()->toISOString(), 'daily', '0.8');
        $xml .= $this->url("{$baseUrl}/video", now()->toISOString(), 'weekly', '0.6');
